...
do
    ...
    do
	gdal_translate     -co "TILED=YES" -co "COMPRESS=DEFLATE" -co "PREDICTOR=2" -co "BIGTIFF=YES"     $SG_URL"/"$var"/"$var"_"$level"_mean_5000.tif"     $GEOTIFF_DATA_DIR"/"$var"_"$level"_mean_5000.tif"
    done
done
